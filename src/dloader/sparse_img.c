...
static u8_t copybuf[COPY_BUF_SIZE];
...
static u32_t total_blocks = 0;

static int read_all(void *buf_src, u32_t src_index, void *buf_dest, size_t len)
{
	memcpy(buf_dest, (void*)((u32_t)buf_src + src_index), len);
	g_buf_index += len;
	return len;
}

static int preread_chunk(void *buf_src, u32_t src_index, void *buf_dest, size_t len)
{
	memcpy(buf_dest, (void*)((u32_t)buf_src + src_index), len);
	return len;
}
...
static int process_crc32_chunk(void *buf, u32_t crc32)
{
	u32_t file_crc32;
	int ret;
...
int write_sparse_img(u8_t * partname, u8_t* buf, unsigned long length)
{
	unsigned int i;
	unsigned int j;
