...
//	struct ubi_device *dev;
	int dev_num;
	//char *cur_volnm;
	char cur_volnm[10 + 1];//UBI_VOL_NAME_MAX
...
typedef struct {
	char name[10 + 1];//UBI_VOL_NAME_MAX
 ...
 *             1 success
 */
OPERATE_STATUS dl_read_midst(uint32_t size, uint32_t off, u8_t * buf);
...
OPERATE_STATUS dl_get_mcptype(u8_t * content, uint16_t * size);
