...
#if 0
static void _decode_packet_data(dl_packet_t *packet, u8_t * partition_name, uint64_t * size, uint32_t * checksum)
{
	uint32_t  i = 0;
...
{
#if 0
	u8_t partition_name[MAX_PARTITION_NAME_SIZE] = {0};
	uint64_t size;
...
int dl_cmd_write_midst(dl_packet_t *packet, void *arg)
{
	...
	_send_reply(1);
	printf("dl_cmd_write_midst %x ",s_iram_address); //every packet
	//debugf("size=0x%x\n", packet->body.size);
	return 0;
}
...
int dl_cmd_read_midst(dl_packet_t *packet, void *arg)
{
	...
	op_res = dl_read_midst(size, off, (u8_t *)(packet->body.content));
...
int dl_cmd_repartition(dl_packet_t *pakcet, void *arg)
{
	...
	u8_t * raw_data = pakcet->body.content;
	REPARTITION_TABLE_INFO rp_info;
	u8_t * p_part_list = NULL;
...
int dl_cmd_write_datetime(dl_packet_t *packet, void *arg)
{
	...
	u8_t * raw_data =(u8_t *) packet->body.content;
